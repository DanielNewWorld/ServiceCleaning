...
import ua.in.cleaning.model.DBHelper;
import ua.in.cleaning.model.DBRequest;
import ua.in.cleaning.model.RequestJSON_CategoryList;
import ua.in.cleaning.network.DataService;
import ua.in.cleaning.requests.GetRequest;

import static ua.in.cleaning.model.DBHelper.keyLogin;
import static ua.in.cleaning.model.DBHelper.keyPass;
import static ua.in.cleaning.model.DBHelper.tableName;
 ...
 * on handsets.
 */
public class MasterDataBaseCategoryFragment extends Fragment {
    ...
    List<RequestJSON_CategoryList> copyList;
    int positionList = 0;
    String login = "", pass = "";
    ...
    public MasterDataBaseCategoryFragment() {
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        //    catalogCatalog = getArguments().getString(ARG_catalogCatalog);
            ...
            CollapsingToolbarLayout appBarLayout = (CollapsingToolbarLayout)
                    activity.findViewById(R.id.tb_Master);
            if (appBarLayout != null) {
                appBarLayout.setTitle("Список категорій"); }*/
        //}
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.master_database_category, container, false);
            ...
            public void onClick(View v) {
                if (copyList.size() != 0) {
                    for (int i = 0; i < copyList.size(); i++) {
                        ...
                        copyList.get(i).categoryName = mtvText.getText().toString();
                    }
                }

                RequestJSON_CategoryList itemList = new RequestJSON_CategoryList("", 0,
                        true);
    ...
    class GetAddListener implements RequestListener<DBRequest> {
        ...
        public void onRequestSuccess(DBRequest dbItems) {
            ...
            switch (chekError) {
                ...
                case "category_dublicat":
                    txtMasterInfo.setText("Така категорія вже існує!");
                    txtMasterInfo.setTextColor(Color.RED);
                    pbMasterNewCatalog.setVisibility(View.INVISIBLE);
                    //Log.d(TAG, "За указанный период списания не найдены" + chekError);
                    Toast.makeText(getContext(),
                            "Така категорія вже існує!", Toast.LENGTH_LONG).show();
                ...
                case "category_YES":
                    ...
                    try {
                        ...
                        List<RequestJSON_CategoryList> arrayList;
                ...
                case "del_dublicat":
                    txtMasterInfo.setText("Цю категорію не можливо видалити, бо вона використовується в каталозі!");
                    txtMasterInfo.setTextColor(Color.RED);
                    pbMasterNewCatalog.setVisibility(View.INVISIBLE);
                    //Log.d(TAG, "За указанный период списания не найдены" + chekError);
                    Toast.makeText(getContext(),
                            "Цю категорію не можливо видалити, бо вона використовується в каталозі!", Toast.LENGTH_LONG).show();
                    break;

                case "category_del_yes":
                ...
                case "category_update_yes":
                    ...
                    try {
                        ...
                        copyList.get(positionList).categoryID = arrayCategory.get(0).categoryID;
                    } catch (UnsupportedEncodingException e) {
                        e.printStackTrace();
                        //} catch (JSONException e) {
                        e.printStackTrace();
    ...
    private void setupRVMaster(@NonNull RecyclerView recyclerView, List<RequestJSON_CategoryList> dbItems) {
                        ...
                        btnYES.setOnClickListener(new View.OnClickListener() {
                            ...
                            public void onClick(View v) {
                                ...
                                try {
                                    ...
                                    jsonObject.put("catalogID", 0);
                                    jsonObject.put("categoryID", mValues.get(position).categoryID);

                                    byte[] data = jsonObject.toString().getBytes("UTF-8");
                                    String data_base64 = Base64.encodeToString(data, Base64.DEFAULT);

                                    positionList = position;
                                    smAdd.execute(new GetRequest("category_del", data_base64),
                                            new GetAddListener());
                                } catch (JSONException |
                ...
                public void onClick(View v) {
                    ...
                    if (login.equals("anonym")) {
                        ...
                        try {
                            ...
                            if (mValues.get(position).categoryName.equals("")) {
                                Toast.makeText(getActivity(), "Ви не ввели назву категорії!",
                                        Toast.LENGTH_LONG).show();
                                txtMasterInfo.setText("Ви не ввели назву категорії!");
                                txtMasterInfo.setTextColor(Color.RED);
                            } else {
                                pbMasterNewCatalog.setVisibility(View.VISIBLE);
                                JSONObject jsonObject = new JSONObject();
                                ...
                                jsonObject.put("catalogID", 0);
                                jsonObject.put("categoryID", mValues.get(position).categoryID);
                                jsonObject.put("categoryName", mValues.get(position).categoryName);

                                byte[] data = jsonObject.toString().getBytes("UTF-8");
                                String data_base64 = Base64.encodeToString(data, Base64.DEFAULT);
                                copyList.get(position).boolCheked = false;
                                positionList = position;
                                smAdd.execute(new GetRequest("category_update", data_base64),
                                        new GetAddListener());
                            }
                        } catch (JSONException |
                                UnsupportedEncodingException e) {
